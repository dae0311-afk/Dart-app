...
def parse_amount(val):
    """
    [버그 수정 1] DART에서 음수를 "(1,234)" 형태로 반환하는 경우가 있음.
    괄호를 마이너스 부호로 변환하도록 수정.
    """
    try:
...
def build_table(year_data):
    ...
    for i, year in enumerate(years):
        ...
        table["Growth"][year]       = fmt_pct((rev / prev_rev - 1) * 100) if (rev and prev_rev and prev_rev != 0) else "-"
...
if "search_results" in st.session_state and not st.session_state["search_results"].empty:
    ...
    if st.button("요약 재무제표 생성", type="primary"):
        """
        [버그 수정 2] 결과를 st.session_state에 저장.
        원래 코드는 버튼 클릭 블록 안에서 모든 결과를 표시했기 때문에,
        이후 CSV 다운로드 버튼 등을 누르면 페이지가 리런되면서 결과가 사라졌음.
        → 결과를 session_state에 저장하고 버튼 블록 밖에서 표시해야 유지됨.
        """
        ...
        for i, year in enumerate(selected_years):
            ...
            for corp_code_iter, corp_name_iter in codes_for_year:
                d, fs_type_used, err = analyze(corp_code_iter, year)
                if d is not None:
                    year_data[year]   = d
                    year_source[year] = corp_name_iter
        ...
        if not year_data:
            ...
            # 결과를 session_state에 저장 → 리런 후에도 유지됨
            st.session_state["result"] = {
                "year_data":   year_data,
                "year_source": year_source,
                "year_fstype": year_fstype,
                "corp_name":   sel,
                "corp_code":   main_corp_code,
            }


# ── 결과 표시 (session_state에서 읽음 → 리런해도 사라지지 않음) ────────────
"""
[버그 수정 2 - 계속]
결과 표시 블록을 버튼 클릭 블록 밖으로 분리.
st.session_state["result"]가 있으면 항상 표시.
"""
if "result" in st.session_state:
    ...
    corp_code_main = r["corp_code"]

    years_sorted = sorted(year_data.keys())

    # 기업 기본 정보
    info = get_corp_info(corp_code_main)
    ...
    with st.expander("교차 검증", expanded=False):
        ...
        for year in years_sorted:
            ...
            vrows.append({
                "연도":             year,
                "조회 법인":        year_source.get(year, sel_name),
