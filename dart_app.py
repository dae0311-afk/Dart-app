...
_INDUTY_MAP = {
    ...
    "49":"육상 운송업","51":"항공 운송업","52":"창고·운송관련 서비스업",
    "55":"숙박업","56":"음식점·주점업","58":"출판업",
    "60":"방송업","61":"통신업",
    "62":"컴퓨터 프로그래밍·시스템 통합업","63":"정보서비스업",
    "64":"금융업","65":"보험·연금업","68":"부동산업",
    "70":"연구개발업","71":"전문 서비스업","72":"엔지니어링 서비스업",
    "75":"사업 지원 서비스업","85":"교육 서비스업",
    "86":"보건업","87":"사회복지 서비스업",
}
...
def download_zip(rcept_no):
    """document.xml ZIP 다운로드. 반환: (html_str, xml_list, pdf_list, meta_dict)"""
    ...
    try:
        ...
        meta["size"] = len(r.content)
        # 에러 XML 응답 감지
        if r.content[:4] == b"<?xm" and b"<result>" in r.content[:400]:
            meta["status"] = "api_error"
            meta["error"]  = r.content[:400].decode("utf-8","ignore")
            return None, [], [], meta
        z = zipfile.ZipFile(io.BytesIO(r.content))
        files = z.namelist()
        meta["files"] = files
...
def _extract_from_text(text, log=None):
    """
    태그 제거 후 순수 텍스트에서 정규식으로 계정명+숫자 추출.
    DART XML, HTML, 텍스트 모두 지원.
    """
    def L(m):
    ...
    # 미리보기 (앞 15줄)
    preview = [ln.strip() for ln in clean.splitlines() if ln.strip()][:15]
    L("    텍스트 미리보기:")
    for p in preview: L(f"      {p[:90]}")
    ...
    for key, kwds in _FIN_KWDS.items():
        ...
        for kw in kwds:
            kw_plain = kw.replace(" ","").replace("(","\\(").replace(")","\\)")
            m = re.search(kw_plain, clean.replace(" ",""))
            if not m: continue
            # 키워드 위치 → 원본 텍스트에서 그 주변 400자
            pos = m.start()
            window = clean.replace(" ","")[pos: pos+400]
            nums = NUM.findall(window)
            # 키워드 자체 글자 수 이후 숫자만
            for raw_n in nums:
                if raw_n in kw.replace(" ",""): continue  # 키워드 안의 숫자 제외
                v = _clean_num(raw_n)
...
def _extract_from_html_table(html_text, log=None):
    """BeautifulSoup HTML 테이블 파싱."""
    def L(m):
        if log is not None: log.append(m)
    try:
        from bs4 import BeautifulSoup
    except:
        L("    beautifulsoup4 미설치"); return {}

    unit = _detect_unit(html_text)
    results = {}
    for parser in ["lxml","html.parser"]:
        try:
            soup = BeautifulSoup(html_text, parser)
            L(f"    파서: {parser}, 단위: {unit:,}")
                        ...
                        if any(label == kw.replace(" ","") or label.startswith(kw.replace(" ","")) for kw in kwds):
                            for cell in cells[1:6]:
                                ...
                                if v is not None and v != 0:
                                    results[key] = v * unit
                                    break
            L(f"    테이블파싱: {len(results)}개")
            break
        except: continue
    return results


def parse_document_content(html_str, xml_list, pdf_list, log=None):
    """
    HTML → XML(텍스트파싱) → PDF 순서로 재무 데이터 추출.
    """
    def L(m):
        if log is not None: log.append(m)
    ...
    if html_str:
        L("  [1] HTML 테이블 파싱")
        results = _extract_from_html_table(html_str, log)
        if "매출액" in results:
            L("  ✅ HTML 파싱 성공"); return results

    # 2. XML — 두 방법 모두 시도
    for fname, xbytes in xml_list:
        L(f"  [2] XML 파싱: {fname} ({len(xbytes):,}bytes)")
        for enc in ["utf-8","cp949","euc-kr"]:
            try:
                ...
                L(f"    인코딩: {enc}")
                # 방법A: HTML 테이블
                r = _extract_from_html_table(xtext, log)
                if "매출액" in r:
                    L("  ✅ XML→HTML테이블 파싱 성공"); return r
                # 방법B: 텍스트 정규식
                r = _extract_from_text(xtext, log)
                if len(r) > len(results): results = r
                if "매출액" in results:
                    L("  ✅ XML→텍스트 파싱 성공"); return results
                break
            except: continue

    # 3. PDF
    if pdf_list:
        L(f"  [3] PDF 파싱 ({len(pdf_list)}개)")
        try:
            import pdfplumber
            for i, pb in enumerate(pdf_list):
                ...
                try:
                    ...
                    if r:
                        ...
                        if unit > 1: r = {k: v*unit for k,v in r.items()}
                    if len(r) > len(results): results = r
                    if "매출액" in results:
                        L("  ✅ PDF 파싱 성공"); return results
                except Exception as e:
                    L(f"    PDF #{i+1} 오류: {e}")
        except ImportError:
            L("  ❌ pdfplumber 미설치")
...
def analyze_from_document(corp_code, year):
    ...
    log.append(f"ZIP: status={meta['status']}, html={meta['html']}, xml={meta['xml']}, pdf={meta['pdf']}, files={meta['files'][:5]}")
    if meta.get("error"):
        log.append(f"  오류: {str(meta['error'])[:200]}")

    raw = parse_document_content(html_str, xml_list, pdf_list, log)

    if not raw or "매출액" not in raw:
        log.append(f"❌ 파싱 실패 — 추출 항목: {list(raw.keys())}")
...
with st.form("search_form"):
    ...
    with c1:
        q = st.text_input("기업명", placeholder="예: 삼성전자, 이노켐", label_visibility="collapsed")
    with c2:
...
if search_btn and q:
    try:
        ...
        if res.empty:
            ...
            st.session_state["sel_idx"] = None   # ← 초기 선택 없음
...
if "result" in st.session_state:
    ...
    if doc_years:
        st.warning(f"📄 {', '.join(doc_years)}년: XBRL 없음 → 공시 문서 파싱. 원문 대조 확인 권장.")
        dbg = r.get("debug_log",{})
