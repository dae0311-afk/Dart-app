...
@st.cache_data(ttl=1800)
def get_fs_list(corp_code, year):
    """공시목록에서 감사보고서/연결감사보고서 조회"""
    r = requests.get(
...
def analyze(corp_code, year):
    """연결 우선, 없으면 별도로 조회. 사업보고서 우선, 없으면 감사보고서 목록 참고"""
    used_fs_type = None
    df = None
    ...
    # 2) 사업보고서 없으면 감사보고서 코드로 재시도 (외감기업 대응)
    if df is None:
...
with st.form("search_form"):
    ...
    with col1:
        q = st.text_input("기업명 입력 (입력 후 엔터 또는 검색 버튼)", placeholder="예: 삼성전자, LG화학")
    with col2:
